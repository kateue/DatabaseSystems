...
WHERE price <= 10.00 and name = 'Amy';
...
--Question 5
SELECT pizzeria, X.name AS person1, X.age AS age1, Y.name AS person2, Y.age as age2
FROM
    (SELECT pizzeria, name, age
    FROM Person JOIN Frequents USING(name) WHERE age<18) AS X
JOIN
    (SELECT pizzeria, name, age
    FROM Person JOIN Frequents USING(name) WHERE age>30) AS Y
    USING (pizzeria);
...
--Question 7
SELECT pizza, AVG(price) AS average_price
FROM Serves
GROUP BY pizza
